...
class LayoutController extends Controller{
     ...
     public function manage()
    {
        ...
        try {
           ...
           $Layout = new Layout; 
            ...
            if (!empty($name)) {
                $Layout->where('name', 'like', '%' . $name . '%');
            }
            // $Layout->where('is', 'like', '%' . $is . '%');
            $layouts = $Layout->paginate($pagesize, false, [
                'query'=>[
                    'name' => $name,
                    ],
                ]);
    ...
    public function save()
    {
        ...
        if($file){
            ...
            if($info){
                ...
                $Layouts = new Layout();
                // 实例化班级并赋值
                $Layouts->path = $info->getSaveName();
                $Layouts->name = Request::instance()->post('name');
                $Layouts->create_time = Request::instance()->post('create_time');
                $Layouts->save();
    ...
    public function update() //接收数据
    {
        ...
        if (!is_null($Layout)) {
            if (!$this->saveLayout($Layout, true)) {
    ...
    private function saveLayout(Layout &$Layout) 
    {
        // 写入要更新的数据
        $Layout->name = Request::instance()->post('name');
        $Layout->path = Request::instance()->post('path');

        // 更新或保存
        return $Layout->validate(true)->save();
    }
    public function delete()
    {
        try {
            ...
            // 删除对象
            if (!$Layout->delete()) {
                return $this->error('删除失败:' . $Layout->getError());
            }
