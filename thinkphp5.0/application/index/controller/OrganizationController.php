...
class OrganizationController extends Controller{
     ...
     public function manage()
    {
        ...
        try {
           ...
           $Organization = new Organization; 
            ...
            // $Organization->where('is', 'like', '%' . $is . '%');
            $organizations = $Organization->paginate($pagesize, false, [
                'query'=>[
                    'name' => $name,
                    ],
                ]);
    ...
    public function save()
    {
        ...
        $file = request()->file('organization');
        ...
        if($file){
            ...
            if($info){
                ...
                // 实例化班级并赋值
                $Organizations->path = $info->getSaveName();
                $Organizations->name = Request::instance()->post('name');
                $Organizations->create_time = Request::instance()->post('create_time');
                $Organizations->save();
    ...
    public function edit()
    {
        try {
            ...
            // 在Organization表模型中获取当前记录
            if (null === $Organization = Organization::get($id))
            {
    ...
    public function update() //接收数据
    {
        ...
        if (!is_null($Organization)) {
            if (!$this->saveOrganization($Organization, true)) {
    ...
    {
        // 写入要更新的数据
        $Organization->name = Request::instance()->post('name');
        $Organization->path = Request::instance()->post('path');

        // 更新或保存
        return $Organization->validate(true)->save();
    }
    public function delete()
    {
        try {
            ...
            if (!$Organization->delete()) {
                return $this->error('删除失败:' . $Organization->getError());
            }
